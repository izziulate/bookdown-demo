...
unit_list <- c("ROMO", "ACAD", "LAKE", "YELL", "GRCA", "ZION", "OLYM", "GRSM")
unit_data_map <- unit_list %>% 
  map(~ unit_visitation(park_code = .))
 

units <- bind_rows(output_data_map)
```
...
```{r}
indiv_vs_parkwide <- visitation %>%
  mutate(singular = ifelse(is.na(UnitName), "Parkwide", "Park")) %>%
  group_by(singular, Year, UnitCode) %>% 
  # we only care about recreational visitors:
  summarize(TotalVisitation = sum(RecreationVisitors))

plotly::ggplotly(
  ggplot(data=indiv_vs_parkwide) +
    geom_point(aes(x = Year, y = TotalVisitation, color = UnitCode)) +
    geom_path(aes(x = Year, y = TotalVisitation, color = UnitCode)) +
    scale_y_continuous(labels = scales::label_scientific()) +
    facet_wrap(~singular, scales = "free_y") +
    theme_bw(base_size=10)
)
```
...
```{r}
unit_percent <- wide_data %>%
  mutate_at(.vars = park_list, .funs = ~(. /Parkwide))
```


```{r}
narrow_percent <- unit_percent %>%
  pivot_longer(cols = -Year,
               names_to = "Park",
               values_to = "RecVisitation") %>%
   filter(!Park %in% c("Parkwide"))

names(narrow_percent)
plotly::ggplotly(
  ggplot(data= narrow_percent, aes(x=Year, y = RecVisitation, color = Park)) +
    geom_point() +
    geom_path() +
    scale_y_continuous(labels = scales::label_scientific())
)
...
```{r}
percent_data <- joined_data %>%
   mutate(
    total_x = NonRecreationVisitors.x + RecreationVisitors.x,
    total_y = NonRecreationVisitors.y + RecreationVisitors.y,
    percent_visitors = total_x / total_y) %>%
    group_by(Year, UnitName.x) %>%
  summarize(percent_visitors = sum(percent_visitors, na.rm=T))
...
plotly::ggplotly(
  ggplot(data= percent_data, aes(x=Year, y = percent_visitors, color = UnitName.x)) +
    geom_point() +
    geom_path() +
    scale_y_continuous(labels = scales::label_scientific())
)
...
```{r}
#Which park on average has the most visitation? 
#Rocky Mountain NP, the percentage of Parkwide visitation is consistently above 6.5%.

#Which park has the least visitation? Base your response on the data starting in 1990, ending in 2021. Defend your answer with numbers!
#Theodore Roosevelt NP, the percentage of Parkwide visitation for this park falls consistently under 2%.
