...
```{r}
one_data <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/total/1980")
two_data <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/total/1999")
three_data <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/total/2018")
```
...
```{r}
four_data <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=ROMO&startMonth=JANUARY&startYear=2010&endMonth=DECEMBER&endYear=2021")
```
...
```{r}
unit_visitation <- function(park_code, 
                              start_month, 
                              start_year, 
                              end_month, 
                              end_year){
# pull in the data
park_data <- httr::GET(url = 
                        # parse out year so that it can be chosen with the "year" argument, using paste0()
                        paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=", park_code, "&startMonth=", start_month,"&startYear=", start_year, "&endMonth=", end_month, "&endYear=", end_year))

# convert content to text
extracted_park_data <- httr::content(park_data, as = "text", encoding = "UTF-8") 

# parse text from JSON to data frame
final_park_data <- jsonlite::fromJSON(extracted_park_data)

return(final_park_data)
...
```{r}
ROMO_data <- unit_visitation( "ROMO", 
                              01, 
                              1990, 
                              12, 
                              2021)

EVER_data <- unit_visitation("EVER", 
                              01, 
                              1990, 
                              12, 
                              2021)
THRO_data <- unit_visitation("THRO", 
                              01, 
                              1990, 
                              12, 
                              2021)
...
ROMO_data_1980_now <- unit_visitation( "ROMO")
EVER_data_1980_now <- unit_visitation( "EVER")
THRO_data_1980_now <- unit_visitation( "THRO")
```
...
```{r}
park_list <- c("ROMO", "EVER", "THRO")

park_data_floop <-vector("list", length = length(park_list))

for(i in 1:length(park_list)){
  
  park_data_floop[[i]] <-
    unit_visitation(park_code = park_list[i])
  
}

summary(park_data_floop)
multi_parks <- dplyr::bind_rows(park_data_floop)
```
...
```{r}
output_data_map <- park_list %>% 
  map(~ unit_visitation(park_code = .))

identical(park_data_floop, output_data_map)

multi_parks <- bind_rows(output_data_map)
